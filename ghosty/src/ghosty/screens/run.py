...
class RunScreen(Screen[None]):
    ...
    def run_actions(self, actions: list[Action]) -> None:
        ...
        self.app.state = evolve(st, mode=AppMode.RUN, previous_mode=st.mode)  # type: ignore[attr-defined]
    ...
    async def action_start(self) -> None:
        ...
        st = self.app.state  # type: ignore[attr-defined]
        dry_run = st.is_dry_run
        max_parallel = st.max_parallel

        self.query_one("#btn-start", Button).disabled = True
        self.query_one("#btn-pause", Button).disabled = False
        self._update_status(f"{'Dry-run' if dry_run else 'Executing'}…")
        self.app.state = evolve(st, is_running=True)  # type: ignore[attr-defined]
        log_event(f"Run start: dry_run={dry_run}, max_parallel={max_parallel}")
        ...
        self.app.state = evolve(  # type: ignore[attr-defined]
            self.app.state,  # type: ignore[attr-defined]
            is_running=False,
            last_run_summary=summary,
        )
        log_event(f"Run complete: {summary}")
